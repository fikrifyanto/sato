@php use App\Models\Pet; @endphp
@extends('customer.layouts.app')

@section('content')
    <div class="max-w-5xl mx-auto px-6 py-8 grid grid-cols-2 gap-8">

        {{-- LEFT: GAMBAR PET --}}
        <div class="bg-white rounded-xl shadow p-4">
            {{-- Gambar Utama --}}
            <div class="rounded-lg overflow-hidden cursor-pointer"
                 onclick="document.getElementById('galleryModal').classList.remove('hidden')">
                <img src="{{ $petData['images'][0] ?? 'https://picsum.photos/600' }}" alt="{{ $petData['name'] }}"
                     class="size-full object-cover">
            </div>

            {{-- Thumbnails --}}
            <div class="flex gap-3 mt-4">
                @foreach ($petData['images'] as $img)
                    <img src="{{ $img }}"
                         class="w-20 h-20 rounded-lg object-cover border cursor-pointer hover:ring-2 hover:ring-amber-500"
                         onclick="document.getElementById('galleryModal').classList.remove('hidden')">
                @endforeach
            </div>
        </div>

        {{-- RIGHT: DETAIL PET --}}
        <div class="bg-white rounded-xl shadow flex flex-col justify-between p-6">
            <div>
                <h2 class="text-xl font-bold text-gray-900 mb-3">{{ $petData['name'] }}</h2>

                {{-- Harga --}}
                <p class="text-amber-600 font-bold text-2xl mb-4">
                    Rp {{ number_format($petData['price'], 0, ',', '.') }}
                </p>

                <div class="text-sm">
                    <p class="text-gray-700 mb-1"><strong>Spesies:</strong> {{ $petData['species'] }}</p>
                    <p class="text-gray-700 mb-1"><strong>Breed:</strong> {{ $petData['breed'] }}</p>
                    <p class="text-gray-700 mb-1"><strong>Gender:</strong> {{ $petData['gender'] }}</p>
                    <p class="text-gray-700 mb-1"><strong>Status Vaksin:</strong> {{ $petData['vaccinated'] }}</p>

                    {{-- Deskripsi --}}
                    <div class="text-gray-700 mb-4">
                        <strong>Deskripsi:</strong>
                        {!! $petData['description'] !!}
                    </div>
                </div>
            </div>

            {{-- Status & Tombol --}}
            <div class="flex gap-2 items-center">
                <button class="w-full bg-amber-600 text-white py-2 rounded-lg hover:bg-amber-700">
                    Adopsi
                </button>
                <form action="{{ route('carts.update') }}" method="POST">
                    @csrf
                    @php $item = $cartItems->where('itemable_type', Pet::class)->where('itemable_id', $petData['id'])->first() @endphp
                    <input type="hidden" name="pet_id" value="{{ $petData['id']  }}">
                    <button @if($item) disabled @endif class="@if($item) opacity-50 @endif bg-amber-600   text-white p-2 rounded-lg hover:bg-amber-700">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2"
                             stroke="currentColor" class="size-6">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                  d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 0 0-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 0 0-16.536-1.84M7.5 14.25 5.106 5.272M6 20.25a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Zm12.75 0a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Z"/>
                        </svg>
                    </button>
                </form>
            </div>
        </div>
    </div>

    {{-- MODAL GALERI --}}
    <div id="galleryModal"
         class="hidden fixed inset-0 bg-black bg-opacity-70 z-50 flex items-center justify-center p-4">

        <div class="bg-white rounded-lg shadow-lg p-4 max-w-3xl w-full relative">

            {{-- Tombol Close --}}
            <button class="absolute top-3 right-3 text-gray-500 hover:text-red-500"
                    onclick="document.getElementById('galleryModal').classList.add('hidden')">
                ✕
            </button>

            {{-- Galeri Besar --}}
            <div class="flex flex-col items-center">
                @foreach ($petData['images'] as $img)
                    <img src="{{ $img }}" class="mb-4 rounded-lg max-h-[500px] object-contain">
                @endforeach
            </div>

        </div>

    </div>
@endsection
